...
export function createScene(canvas) {
  ...
  /* The can hangs off a second, inner node, and this is not
     decoration. `spin` has to mean "the can turning about its own
     axis" in every section — but the hero lays the can on its side,
     and a yaw applied to a can that is already rolled would swing it
     end-on to the camera instead of rolling it. Putting the roll on
     the inner node makes it axis-relative, so the same number reads
     as a spin standing up and as a roll lying down.

     Outer: position, scale, pitch (rotX), roll-to-lying (rotZ).
     Inner: the can's own turn (rotY). */
  ...
  /* The drink. Solid rather than translucent — an energy drink is
     ...
     Filled to well above the sleeve's top edge: the sleeve ends at
     world y ≈ 1.15, and a can filled to below that is a can whose
     drink you cannot see. The level wants to land in the clear
     window above the print, where the meniscus reads. */
  const FILL_Y = 1.62;
  const liquidMat = new THREE.MeshPhysicalMaterial({
    color: new THREE.Color(FLAVORS[0].liquid),
    // Lit from within, but only just. The accent is a neon and the
    // liquid it is tinting is already saturated, so the emissive is
    // there to stop the drink going flat in shadow — push it much
    // past this and the can reads as a glow stick rather than as
  ...
  const liquid = new THREE.Mesh(buildLiquidGeometry(FILL_Y), liquidMat);
  ...
  /* The printed band. Same lathe as the wall, pushed out a hair so
     ...
     The coat is doing more work here than on the glass, because the
     label underneath is dark. A droplet over black has no diffuse
     response to brighten it — all that makes it read as water is the
     specular it catches, and that has to come from a near-mirror
     coat rather than from the base layer. The normal scale is kept
     low for the same reason: a strong normal on black just digs a
     deeper dark side into the bead. */
  ...
  function update(dt) {
    ...
    // The can's own turn lives on the inner node so it stays relative
    // to the can rather than to the world (see the note at `roll`).
    roll.rotation.y = current.rotY + pointer.x * 0.30;
  ...
  /* Park the can for the entrance.

     The pose — where it sits and how it is rolled — is snapped onto the
     current target rather than damped, because the entrance is not
     meant to include it: `applyPreset('hero')` lays the can down, and
     easing that from the default upright spends the opening second and
     a half swinging a full-size can through a quarter turn across the
     wordmark, which reads as a mistake rather than as an arrival.

     Scale and spin are the entrance, and both are set on `current`
     only. Touching `target` here is what makes an entrance invisible:
     the preset that just ran already put the real values there, so
     overwriting them would leave the damping with nowhere to travel. */
  function park() {
    current.x = target.x;
    current.y = target.y;
    current.rotX = target.rotX;
    current.rotZ = target.rotZ;

    // A whole number of turns out, so unwinding lands on the FRONT of
    // the label rather than on its back.
    current.rotY = target.rotY - Math.PI * 2;
    current.scale = 0.15;
  }
  ...
  return {
    ...
    material: bandMat,
    // The transform nodes, so tooling can read back what the damping
    // actually settled on instead of inferring it from a screenshot.
    nodes: { group, roll },
  };
}
