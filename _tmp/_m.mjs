...
/* ── Can metrics ────────────────────────────────────────────
   ...
   index, not by arc length, so this 84-point layout is what pins the
   printed band to v 9/83 … 69/83. can-label.js bakes those same two
   numbers into the canvas, so none of the counts below can move
   without the artwork sliding out of place with them. The Y values
   and radii, by contrast, are free to move.
   ─────────────────────────────────────────────────────────── */
export const CAN = {
  bodyR: 1.0,
  bodyTop: 2.05,
...
function tabShape() {
  ...
  shape.moveTo(-0.070, 0.0);
  shape.splineThru([
    new THREE.Vector2(0.000, 0.055),
    new THREE.Vector2(0.105, 0.060),
    new THREE.Vector2(0.215, 0.056),
    new THREE.Vector2(0.320, 0.068),
    new THREE.Vector2(0.400, 0.070),
    new THREE.Vector2(0.455, 0.0),
    new THREE.Vector2(0.400, -0.070),
    new THREE.Vector2(0.320, -0.068),
    new THREE.Vector2(0.215, -0.056),
    new THREE.Vector2(0.105, -0.060),
  ...
  hole.absarc(0.335, 0, 0.050, 0, Math.PI * 2, true);
...
export function buildTab() {
  ...
  const rivet = new THREE.CylinderGeometry(0.040, 0.048, 0.052, 20)
    .translate(RIVET_X, PANEL_Y + 0.026, 0);

  const button = new THREE.SphereGeometry(0.040, 20, 10, 0, Math.PI * 2, 0, Math.PI / 2)
    .translate(RIVET_X, PANEL_Y + 0.052, 0);
