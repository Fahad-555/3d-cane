...
/* The printed band, as fractions of canvas height. These mirror the
   lathe's point indexing in scene.js (9/83 … 69/83). They are
   duplicated rather than imported because scene.js is the module
   that imports *this* one, and a cycle there would be fatal.
   Outside the band the surface has already begun curving into the
   shoulder, where a straight rule would visibly bend. */
export const BAND = { y0: 0.169, y1: 0.892 };
...
/* ── The front composition ───────────────────────────────────
   ...
   drawn once per seam. Left to right it is: two mono spec columns,
   the vertical wordmark, the shard. The shard sits slightly right of
   the seam and the wordmark slightly left, which keeps both inside
   the arc while still reading as one centred panel. */
function drawFrontPanel(ctx, cx, S, y0, bh, FRONT, flavor) {
  ...
  // Flavour index and name, set small along the top of the band.
  const cap = fit(ctx, flavor.name.toUpperCase(), F * 0.62, bh * 0.030, 500, MONO);
  ctx.font = font(500, cap, MONO);
  ctx.fillStyle = accent;
  ctx.fillText(flavor.name.toUpperCase(), cx, y0 + bh * 0.088);

  ctx.font = font(400, fit(ctx, 'VOLTIC BEVERAGE CO. · ROTTERDAM', F * 0.72, bh * 0.022, 400, MONO), MONO);
  ctx.fillStyle = rgba(ink, 0.42);
  ctx.fillText('VOLTIC BEVERAGE CO. · ROTTERDAM', cx, y0 + bh * 0.132);

  // Two mono spec columns to the left of the wordmark. Each is short
  // enough to stay inside the arc on its own; stacking them
  // vertically instead would run them off the band.
  const colA = cx - F * 0.405;
  const colB = cx - F * 0.470;

  ctx.fillStyle = rgba(ink, 0.88);
  verticalText(ctx, 'ENERGY DRINK', colA, y0 + bh * 0.300,
    fit(ctx, 'ENERGY DRINK', bh * 0.30, F * 0.048, 500, MONO), MONO, 500, null, 0);

  ctx.fillStyle = rgba(ink, 0.55);
  verticalText(ctx, '200MG CAFFEINE', colB, y0 + bh * 0.300,
    fit(ctx, '200MG CAFFEINE', bh * 0.30, F * 0.042, 400, MONO), MONO, 400, null, 0);
  ctx.fillStyle = rgba(ink, 0.45);
  verticalText(ctx, 'ZERO SUGAR', colB, y0 + bh * 0.660,
    fit(ctx, 'ZERO SUGAR', bh * 0.16, F * 0.042, 400, MONO), MONO, 400, null, 0);
  ...
  const wmSize = fit(ctx, 'VOLTIC', bh * 0.58,
    Math.min((F * 0.25) / 0.73, bh * 0.30), 400, DISPLAY);
  verticalText(ctx, 'VOLTIC', cx - F * 0.245, y0 + bh * 0.440, wmSize, DISPLAY, 400,
    [[0.00, '#ffffff'], [0.42, ink], [1.00, accent]], wmSize * 0.05);

  /* Shard, centred just right of the seam. The box is wider than it
     is tall relative to a bolt's natural aspect: the readable arc is
     only ~570px across while the band is ~1480 tall, so the mark is
     designed fat and then squashed into the arc, which is also what
     stops its strokes reading as two thin spikes. */
  drawShard(ctx, cx + F * 0.195, y0 + bh * 0.460, F * 0.400, bh * 0.225, accent);

  // Spec line under it all, fitted to the arc like every other string.
  const spec = fit(ctx, '500 ML · 200 MG CAFFEINE', F * 0.88, bh * 0.040, 500, MONO);
  ctx.font = font(500, spec, MONO);
  ctx.fillStyle = rgba(ink, 0.90);
  ctx.fillText('500 ML · 200 MG CAFFEINE', cx, y0 + bh * 0.845);

  ctx.font = font(400, spec * 0.78, MONO);
  ctx.fillStyle = rgba(ink, 0.42);
  ctx.fillText('SUGAR FREE · VEGAN · 10 CAL', cx, y0 + bh * 0.895);
}
